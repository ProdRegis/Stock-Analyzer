...
The dashboard is organized into nine tabs, grouped as research
(Portfolio, Thesis, Options, News) and find-names (Breakouts, Dips & Shorts,
Stops, Copy Trading, Vol screen).
...
**Vol screen** — A liquid optionable book, not a list of stocks to buy. It
sweeps index/sector ETFs (SPY, QQQ, IWM, and the rest) plus mega-caps that
actually have a tight ATM market, then ranks them by bid–ask / open interest
first and IV versus 30-day RV second. Earnings prints and the no-trade IV/RV
band are not “best trades.” Click a row to open the Options desk.
...
a VRP sale — the desk offers a Robinhood Level 3 Long straddle versus
stand-aside, and sizes the implied move to the print date. Default expiry is
~21–45 DTE; 0 DTE and front-week listings stay behind a weeklies toggle. Click
calls and puts on the chain to build a custom structure (ask to buy, bid to
sell). Every named structure is a ticket you can submit in Robinhood
**Trade → Trade options → Strategy builder**. Set Level 2 (long call/put,
covered call, cash-secured put) or Level 3 (margin: straddles, verticals, iron
condor). Naked shorts are never recommended — Robinhood will not take them.
A **Recommendation** names one of those tickets — or sit out — using expected
expiration P&L (typical win × chance + typical miss × chance), not a high
win-rate score. Compare cards still show **best match**, **highest chance**,
and **highest expected return**. The desk sits out the no-trade IV/RV band and
earnings lotteries. Calendars and diagonals are omitted because this desk is
single-expiry.
Charts show expiration P&L plus a mark-to-model path at current IV and a
+5 vol-point shock. Holdings get a book scan of IV/RV and term shape; click a
row to open that chain. Theta is labeled as the rent for gamma, not an edge.
Naked short calls are not the recommended expression of a short-vol view. This
is not a broker and does not estimate Interactive Brokers margin.
...
Screenshots are handled without an OCR service: type the positions as JSON
(ticker, shares, optional average cost) or copy a table from a spreadsheet
and paste it in. Parsing real text is more reliable than reading pixels.
...
The scanner, batch, 13F, options-desk, and vol-screen endpoints fan out or parse large filings, so they
