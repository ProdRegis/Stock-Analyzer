...
function formatPop(value: number | null) {
  if (value == null || !Number.isFinite(value)) return "—";
...
function LevelToggle({
  ...
  onChange: (level: RobinhoodLevel) => void;
}) {
  return (
    <div
      className="flex rounded-lg border border-slate-700 p-0.5 text-xs"
      role="group"
      aria-label="Robinhood options approval level"
    >
...
function RecommendationPanel({
  ...
  rec: BeginnerRecommendation;
  active: boolean;
  level: RobinhoodLevel;
  onLevelChange: (level: RobinhoodLevel) => void;
  onSelect: (id: OptionStructureView["id"]) => void;
}) {
  const ranked = rec.ranked;
  const sitOut = rec.action === "stand_aside";
  const howTo = ranked ? robinhoodHowTo(ranked.structure.id) : null;

  return (
    <section className="surface-2 rounded-2xl p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h3 className="text-sm font-medium text-slate-200">Recommendation</h3>
          <p className="mt-1 text-xs text-slate-500">
            One Robinhood ticket at your approval level, or sit out. Level 2 is
            Long call, Long put, Covered call, and Cash-secured put. Level 3
            (margin) adds straddles, verticals, and Iron condor. Path:{" "}
            {ROBINHOOD_PATH}. Limit orders only — they will not take a naked
            short.
          </p>
        </div>
      ...
      >
        <p className="text-base font-medium text-white">{rec.headline}</p>
        {howTo && (
          <p className="mt-1 text-xs text-blue-200/80">{howTo}</p>
        )}
        <p className="mt-2 text-sm leading-relaxed text-slate-300">{rec.why}</p>
        {ranked && (
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div>
              <p className="text-[11px] uppercase tracking-wide text-slate-500">
                Chance the chain prices
              </p>
              <p className="mt-1 text-sm text-white">{formatPop(ranked.pop)}</p>
              <p className="mt-0.5 text-[11px] text-slate-500">
                P(expiration P&amp;L &gt; 0) under ATM IV
              </p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <p className="mt-1 text-sm text-white">
                {formatMoney(ranked.typicalWin, 0)} /{" "}
                {formatMoney(ranked.typicalLoss, 0)}
              </p>
              <p className="mt-0.5 text-[11px] text-slate-500">
                If the last 30 days of movement continue
              </p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <p className="mt-1 text-sm text-white">
                {formatMoney(ranked.expectedPnl, 0)}
                {ranked.expectedReturn != null
                  ? ` · ${formatReturn(ranked.expectedReturn)}`
                  : ""}
              </p>
              <p className="mt-0.5 text-[11px] text-slate-500">
                Typical win × its chance + typical miss × its chance
              </p>
            </div>
          </div>
        )}
        ...
        <p className="mt-3 text-xs leading-relaxed text-slate-500">{rec.trap}</p>
        {ranked && (
          <button
            type="button"
            onClick={() => onSelect(ranked.structure.id)}
            className={`mt-4 rounded-full border px-3 py-1.5 text-xs font-medium transition ${
              active
                ? "border-blue-500/40 bg-blue-500/15 text-blue-200"
                : "border-slate-600 text-slate-300 hover:border-slate-400 hover:text-white"
            }`}
          >
            {active ? "Showing this structure" : `Open ${ranked.structure.name}`}
          </button>
        )}
      </div>
    </section>
...
function PickCard({
  ...
  return (
    <button
      ...
      disabled={!ranked}
      onClick={() => {
        if (ranked) onSelect(ranked.structure.id);
      }}
    ...
    >
      ...
      <p className="mt-1 text-sm font-medium text-white">
        {ranked?.structure.name ?? "None"}
      </p>
      {ranked && (
        <p className="mt-1 text-xs text-slate-400">
          Win {formatPop(ranked.pop)} · E[return] {formatReturn(ranked.expectedReturn)}
        </p>
      )}
    </button>
  );
      ...
      {scan && (
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[640px] text-xs">
            ...
            <tbody>
              {sortScanRows(scan.rows).map((row, index) => {
                const bestBook =
                  index === 0 && !row.skipped && row.stance !== "wait";
                return (
                <tr
                  key={row.symbol}
...
function DeskReport({
  ...
  const [rhLevel, setRhLevel] = useState<RobinhoodLevel>(3);
  ...
  const structures = useMemo(
    () => (custom ? [custom, ...desk.structures] : desk.structures),
  ...
  const picks = useMemo(
    () => selectPicks(ranked, desk.stance, rhLevel),
    [ranked, desk.stance, rhLevel]
  );
  const recommendation = useMemo(
    () => recommendForBeginner(ranked, desk.stance, rhLevel),
    [ranked, desk.stance, rhLevel]
  );
  const customRobinhood = custom
    ? robinhoodAllowsDraft(draft, custom.definedRisk)
    : null;
  const selectedRank =
    ranked.find((row) => row.structure.id === selected?.id) ?? null;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              ...
              Buy uses the ask; sell uses the bid — same as a Robinhood limit.
              A lone short call is not a ticket (use Covered call or a Call
              credit spread). Front-week expiries stay hidden until you turn
              weeklies on.
            </p>
          </div>
        ...
        {custom && (
          <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-slate-700/80 bg-slate-900/50 px-3 py-2 text-xs text-slate-300">
            ...
            {customRobinhood && (
              <span
                className={
                  customRobinhood.allowed
                    ? "text-emerald-300/80"
                    : "text-red-300"
                }
              >
                {customRobinhood.allowed
                  ? "Robinhood can take this"
                  : "Robinhood will reject this"}
              </span>
            )}
            <button
              type="button"
      ...
      {structures.length > 0 && (
        <RecommendationPanel
          rec={recommendation}
          active={selected?.id === recommendation.ranked?.structure.id}
          level={rhLevel}
          onLevelChange={setRhLevel}
      ...
      {structures.length > 0 && (
        <section className="surface-2 rounded-2xl p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">{picks.note}</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <PickCard
              label="Best match"
              ranked={picks.best}
              active={selected?.id === picks.best?.structure.id}
              onSelect={setSelectedId}
            />
            <PickCard
              label="Highest chance"
              ranked={picks.highestChance}
              active={selected?.id === picks.highestChance?.structure.id}
              onSelect={setSelectedId}
            />
            <PickCard
              label="Highest expected return"
              ranked={picks.highestReturn}
              active={selected?.id === picks.highestReturn?.structure.id}
              onSelect={setSelectedId}
            />
          </div>
        </section>
      ...
      {structures.length > 0 && (
        <section className="surface-2 rounded-2xl p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Names match Robinhood {ROBINHOOD_PATH}. The highlighted card
            matches the vol stance. Level {rhLevel} tickets stay bright; higher
            levels are still viewable. Premiums use bid to sell and ask to buy.
            One contract, multiplier 100. Not a broker — no live margin.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            {structures.map((structure) => {
              const active = structure.id === selected?.id;
              const ticket = robinhoodTicket(structure.id);
              const fits =
                structure.id === "custom"
                  ? rhLevel === 3 &&
                    structure.definedRisk &&
                    (customRobinhood?.allowed ?? false)
                  : ticketFitsLevel(structure.id, rhLevel);
              return (
                <button
                  key={structure.id}
                  type="button"
          ...
          {selected && (
            <div className="mt-5 grid gap-6 lg:grid-cols-2">
              <div>
                ...
                {robinhoodHowTo(selected.id) && (
                  <p className="mt-1 text-xs text-slate-500">
                    {robinhoodHowTo(selected.id)}
                    {(() => {
                      const ticket = robinhoodTicket(selected.id);
                      if (!ticket) return null;
                      if (ticket.needs === "shares") {
                        return " · needs 100 shares";
                      }
                      if (ticket.needs === "cash") {
                        return " · cash-secured";
                      }
                      if (ticket.needs === "margin") {
                ...
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="text-slate-200">
                      {formatPop(selectedRank?.pop ?? null)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-slate-500">
                    ...
                    <dd className="text-slate-200">
                      {formatReturn(selectedRank?.expectedReturn ?? null)}
                    </dd>
                  </div>
                </dl>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-400">
              ...
              measures realized vol, then names a Robinhood{" "}
              {ROBINHOOD_PATH} ticket — or sit out. It will not tell you to buy
              a call because you are bullish, and it will not suggest a naked
              short.
            </p>
          </div>
          {onOpenVolScreen && (
            <button
              type="button"
              onClick={onOpenVolScreen}
              className="rounded-full border border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:border-slate-500 hover:text-white"
            >
              Find liquid names
            </button>
          )}
      ...
      {!loading && !desk && !error && (
        <EmptyState
          ...
          description="Search a name with listed options, or click a holding in the book scan. You will get ATM IV versus realized vol, the term structure, skew, a stance, and Robinhood Strategy builder tickets sized to that view."
        />
      )}
