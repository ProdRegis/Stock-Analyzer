 ...
 *
 * Runs in the browser with no API call. Accepts JSON, what a spreadsheet
 * copies, and what selecting a brokerage table on a web page produces.
 */

import { buildImportResult, type ImportResult } from "./portfolio-import";

/** Sample JSON users can fill in from a screenshot or statement. */
export const HOLDINGS_JSON_EXAMPLE = `{
  "holdings": [
...
/** Accepts JSON, CSV, TSV, or a table copied off a web page. */
export function parsePastedHoldings(text: string): ImportResult {
  ...
  // A copied JSON blob is the most precise input, so try it first and only
  // fall back to line parsing when it is not JSON.
