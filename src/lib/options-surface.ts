...
export function volStance(input: StanceInput): VolStanceResult {
  ...
  if (input.earningsInWindow) {
    ...
    reasons.push(
      "Robinhood Level 3 Long straddle if you think realized |move| beats the implied move; otherwise stand aside. Do not sell a condor through the print — that is not a Level 2 cash-secured put."
    );
  ...
  if (ratio >= SELL_RATIO) {
    ...
    reasons.push(
      preferred === "iron_condor"
        ? "On Robinhood this is Trade → Trade options → Strategy builder → Iron condor (Level 3). They will not take a naked short straddle."
        : "Not enough clean strikes for a condor — Robinhood Level 3 Put credit spread is the defined-risk substitute. Naked shorts are not a ticket."
    );
  ...
  if (ratio <= BUY_RATIO) {
    ...
    reasons.push(
      "On Robinhood this is Trade → Trade options → Strategy builder → Long straddle (Level 3, margin). A Long call is Level 2 but that is a stock bet, not this IV/RV view. You pay theta for positive gamma — the move (or an IV expansion) has to more than cover that rent."
    );
