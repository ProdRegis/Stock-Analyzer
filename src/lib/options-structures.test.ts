...
describe("expirationStats", () => {
  ...
  it("caps a call credit spread — Robinhood's naked-call substitute", () => {
    const stats = expirationStats(callCreditSpread(105, 1.7, 110, 0.7));
    expect(stats.debitCredit).toBe("credit");
    expect(stats.definedRisk).toBe(true);
    expect(stats.netPremium).toBeCloseTo(1, 8);
    expect(stats.maxProfit).toBeCloseTo(100, 4);
    expect(stats.maxLoss).toBeCloseTo(-400, 4);
  });

  it("caps a put debit spread", () => {
    const stats = expirationStats(putDebitSpread(100, 4, 90, 1.5));
    expect(stats.debitCredit).toBe("debit");
    expect(stats.definedRisk).toBe(true);
    expect(stats.netPremium).toBeCloseTo(2.5, 8);
    expect(stats.maxLoss).toBeCloseTo(-250, 4);
